...
function codeLane(holder, width, height, data, custom_options) {
  ...
  var options = {
    zoom: {
      ...
      wedgeHeight: 50,
      targetHeight: 100,
      stroke: "rgba(74,144,226,.4)",
      fill: "rgba(74,144,226,.05)",
      text: "rgba(74,144,226,1)"
    },
    marker: {
      size: 10
    },
  ...
  function drawMarker(holder, marker, hover_mode) {
    ...
    switch (marker.shape) {
      case "diamond":
        ...
        break;
      case "triangle":
        holder.graphics.clear().s(strokeColor).ss(1, "round").beginFill(fillColor).drawPolyStar(0, 0, options.marker.size / 2, 3, 0, 270);
        break;
      case "hexagon":
        holder.graphics.clear().s(strokeColor).ss(1, "round").beginFill(fillColor).drawPolyStar(0, 0, options.marker.size / 2, 6, 0, 270);
        break;
      case "pentagon":
        holder.graphics.clear().s(strokeColor).ss(1, "round").beginFill(fillColor).drawPolyStar(0, 0, options.marker.size / 2, 5, 0, 270);
        break;
      case "rect":
        let width = options.marker.size;
        let height = ( marker.endloc - marker.loc ) * locScale;
        height = height > width ? height : width;
        holder.graphics.clear().s(strokeColor).ss(1, "round").beginFill(fillColor).drawRect(-width/2, 0, width, height);
        break;
      case "square":
  ...
  function handleMarkerHover(event) {
    ...
    // if(!(typeof event.target.data.hover !== "undefined" && event.target.data.hover === false))
    // {
    //   return false;
    // }
    ...
    if (label.getBounds().width > (availabletextHeight * 70 / 100)) {
      ...
      createjs.Tween.get(label, {
        ...
        y: label.y - diff
      }, 3000, createjs.Ease.getPowInOut(2));
    }
    ...
    createjs.Tween.get(blade_wrapper).to({
      alpha: 1
  ...
  function handleMarkerOut(event) {
    ...
    createjs.Tween.get(event.target.data.blade_wrapper, {
        onComplete: function () {
          event.target.parent.removeChild(event.target.data.blade_wrapper);
        }
      })
      .to({
        alpha: 0
  ...
  function handleMarkerClick(event) {
    ...
    switch (event.target.data.shape) {
      case "diamond":
        ...
        break;
      case "triangle":
        dummy_marker.graphics.clear().beginFill(fillColor).drawPolyStar(0, 0, options.marker.size / 2, 3, 0, 270);
        break;
      case "pentagon":
        dummy_marker.graphics.clear().beginFill(fillColor).drawPolyStar(0, 0, options.marker.size / 2, 5, 0);
        break;
      case "hexagon":
      ...
      case "rect":
        let width = options.marker.size;
        let height = ( data.endloc - data.loc ) * locScale;
        height = height > width ? height : width;
        // holder.graphics.clear().s(strokeColor).ss(1, "round").beginFill(fillColor).drawRect(-width/2, 0, width, height);
        dummy_marker.graphics.clear().beginFill(fillColor).drawRect(-width/2, 0, width, height);
        break;
      default:
    ...
    createjs.Tween.get(dummy_marker, {
        onComplete: function () {
          event.target.parent.removeChild(dummy_marker);
        }
      })
      .to({
        alpha: 0,
